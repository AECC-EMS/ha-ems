...
class AeccSwitch(SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        _LOGGER.info(f"按钮开启{self.device.is_local}")
        _LOGGER.debug(f"{self.device.icon_type}")
        if self.device:
            if self.device.is_local:
                _LOGGER.info(f"本地设置")
                await self.hub.switch_local(self.device, True)
            else:
                _LOGGER.info(f"云端设置")
    ...
    async def async_turn_off(self, **kwargs):
        ...
        _LOGGER.debug(f"{self.device.icon_type}")
        _LOGGER.info(f"按钮关闭{self.device.is_local}")
        if self.device.is_local:
            _LOGGER.info(f"本地设置")
            await self.hub.switch_local(self.device, True)
        else:
