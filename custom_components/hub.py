...
class MyIntegrationHub:
    def __init__(self, hass, username, password, senceId):
        ...
        self.energy_data_year = None
        self.energy_data_total = None
        self.energy_data_month = None
        self.energy_data_day = None
    ...
    async def async_update_data(self, now=None):
        ...
        try:
            ...
            # await self.get_energy_data_day(self.senceId)
            # await self.get_energy_data_month(self.senceId)
            # await self.get_energy_data_year(self.senceId)
            # await self.get_energy_data_total(self.senceId)
    ...
    async def switch_local(self, device, v):
        # _LOGGER.info(self._session)
        url = "http://"+device.local_ip+"/aecc/"+device.local_type+"/post"
    ...
    async def get_energy_data_day(self, plant_id, sn=""):
        ...
        res = resp['obj']
        self.energy_data_day=res
        for entity in self._entities:
            entity.update_data({"plant_name":''}, self.devices_info, None)
        # _LOGGER.info(res)
        return res
